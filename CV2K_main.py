...
if __name__ == '__main__':
    ...
    eps = 1e-6
    # create rank cycle
    rank_cycle = np.array([[i] * args.reps for i in np.arange(args.bottom_k, args.top_k+1, args.stride)]).flatten()
    Ks = np.unique(rank_cycle)
    ...
    for k in range(best_k_arg):
        ...
        if p > 0.05:
            best_k_arg = k
            break
    best_k_after_rollback = Ks[best_k_arg]
